...
const coinKeys = {
  ...
  list: (query: string, mode?: Mode, sortBy?: SortBy) =>
    [...coinKeys.lists(), { query, mode, sortBy }] as const,
...
export const useCoin = (
  id: number,
  options?: Omit<UseQueryOptions<api.CoinResp>, 'queryKey' | 'queryFn'>,
) => {
  return useQuery({
...
export const useSearchCoins = (
  ...
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      api.searchCoins(searchQuery, mode, sortBy, limit, pageParam),
...
export const useGenerateCoin = (
  options?: Omit<
    UseMutationOptions<
      api.CoinResp,
      Error,
      api.RandomCoinReq | api.PromptCoinReq | api.SocialCoinReq
    >,
    'mutationFn'
  >,
) => {
  ...
  return useMutation({
    mutationFn: (data: api.RandomCoinReq | api.PromptCoinReq | api.SocialCoinReq) =>
